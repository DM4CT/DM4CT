...
from utils_inr import SIREN, PositionalEncoder, create_grid_3d
...
save_dir = f"inr/{dataset_name}/{num_angles}projs/"
intermediate_dir = f"inr/{dataset_name}/{num_angles}projs/intermediate"
...
for i in range(len(dataset)):    
    ...
    grid = create_grid_3d(n_slices,n_rows,n_cols).to(device)
    ...
    for epoch in tqdm(range(epochs)):
        ...
        train_projs = A(train_output.squeeze(-1))
